               ...
               REAL *psi, REAL *psi0)
{
  ...
  coef_diff(para, var, psi, psi0, var_type);

  // Calculate Source Term
 // source_diff(para, var, var_type);    

  // Solve the equations
  equ_solver(para, var, var_type, psi);

  // Define B.C.
  set_bnd(para, var, var_type, psi);
...
void source_diff(PARA_DATA *para, REAL **var, int var_type)
{
  ...
  FOR_EACH_CELL
    switch(var_type)
    {
      ...
      case TEMP: 
        b[IX(i,j,k)] += var[TEMPS][IX(i,j,k)]; break;
      case DEN:  
        b[IX(i,j,k)] += var[DENS][IX(i,j,k)];  break;
    }
  END_FOR
} // End of source_diff()
