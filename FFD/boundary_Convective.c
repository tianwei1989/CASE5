...
void set_bnd_temp(PARA_DATA *para, REAL **var, int var_type, REAL *psi)
{
  ...
  if(k<=k3) 
  {
	  psi[IX(i,j-1,k)]= para->bc->T_bcS; /*south wall*/
	  as[IX(i,j,k)]= coeff_h*(gx[IX(i,j,k)]-gx[IX(i-1,j,k)])*(gz[IX(i,j,k)]-gz[IX(i,j,k-1)]);
  ...
  if(k>k1) 
  {
	  psi[IX(i,j+1,k)]= para->bc->T_bcN; /*north wall*/
      an[IX(i,j,k)]= coeff_h*(gx[IX(i,j,k)]-gx[IX(i-1,j,k)])*(gz[IX(i,j,k)]-gz[IX(i,j,k-1)]);
  }
  else 
  {	  
  ...
  aw[IX(i,j,k)]= coeff_h*(gy[IX(i,j,k)]-gy[IX(i,j-1,k)])*(gz[IX(i,j,k)]-gz[IX(i,j,k-1)]);
  END_FOR

  i=imax;
  FOR_JK
  psi[IX(i+1,j,k)]=para->bc->T_bcE;
  ae[IX(i,j,k)]= coeff_h*(gy[IX(i,j,k)]-gy[IX(i,j-1,k)])*(gz[IX(i,j,k)]-gz[IX(i,j,k-1)]);
  END_FOR
 
  k=1;
  FOR_IJ
  psi[IX(i,j,k-1)]=para->bc->T_bcB;
  ab[IX(i,j,k)]= coeff_h*(gy[IX(i,j,k)]-gy[IX(i,j-1,k)])*(gx[IX(i,j,k)]-gx[IX(i-1,j,k)]);
  END_FOR

  k=kmax;
  FOR_IJ
  psi[IX(i,j,k+1)]=para->bc->T_bcT;
  af[IX(i,j,k)]= coeff_h*(gy[IX(i,j,k)]-gy[IX(i,j-1,k)])*(gx[IX(i,j,k)]-gx[IX(i-1,j,k)]);
  END_FOR
  ...
  -------------------------------------------------------------------------- */
   ...
   j = j2;    
			...
			for(k=1;k<=k2; k++)
	 {
	  ...
	  as[IX(i,j+1,k)]= coeff_h*(gx[IX(i,j+1,k)]-gx[IX(i-1,j+1,k)])*(gz[IX(i,j+1,k)]-gz[IX(i,j+1,k-1)]);
	 }
  ...
  for(j=j1+1;j<=j2; j++)
	 ...
	 {
         psi[IX(i,j,k)] = para->bc->T_bcBOX;
         aw[IX(i+1,j,k)]= coeff_h*(gy[IX(i+1,j,k)]-gy[IX(i+1,j-1,k)])*(gz[IX(i+1,j,k)]-gz[IX(i+1,j,k-1)]);
	 }
  ...
  for(i=i1+1; i<=i2; i++)
	...
	{
         psi[IX(i,j,k)] = para->bc->T_bcBOX;
         ab[IX(i,j,k+1)]= coeff_h*(gy[IX(i,j,k+1)]-gy[IX(i,j-1,k+1)])*(gx[IX(i,j,k)]-gx[IX(i-1,j,k+1)]);
	}
...
void mass_conservation(PARA_DATA *para, REAL **var)
{
  ...
  REAL H_out=para->geom->z1; 
  REAL L_out= para->geom->Lx;
    ...
    ---------------------------------------------------------------------------*/
      ...
      FOR_KI
		if(k<=k1 )
		{
			...
			v[IX(i,j,k)] = v[IX(i,j,k)]+(mass_in-mass_out)/(H_out*L_out);
		}
      END_FOR
